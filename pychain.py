...
if len(pychain.chain)>1: 
    pychain_list=[]
    for each_block in pychain.chain[1:]: 
        pychain_list.append([each_block.record.sender, 
                             each_block.record.receiver, 
                             each_block.record.amount, 
                             each_block.creator_id, 
                             each_block.prev_hash, 
                             each_block.nonce, 
                             each_block.timestamp])
    pychain_df = pd.DataFrame(pychain_list, 
                              columns=['sender', 
                                       'receiver', 
                                       'amount', 
                                       'creator_id', 
